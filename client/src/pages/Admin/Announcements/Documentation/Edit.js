...
export default function AdminEduTour() {
  ...
  const documentationReducerData = useSelector(
    (state) => state.documentationReducer
  );
  const isSuccess = useSelector(
    (state) => state.documentationReducer.successful
  );
  useEffect(() => {
  ...
  let documentations = documentationReducerData.documentations;
  const deleteDocumentation = (id, filename) => {
    setDel(true);
    const delOldImg = storageRef.child("Documentations" + "/" + filename);
    // Delete the file
    delOldImg
      .delete()
      .then(() => {
        dispatch(deleteData("Documentations", id));
        setAction("Delete");
        setOpen(true);
        setDel(false);
      })
  ...
  return (
    <div>
      ...
      {/* <ul>
        {documentations.map(({ _id, title, description }) => (
      ...
      {isSuccess == "true" ? (
        <Snackbar open={open} autoHideDuration={6000} onClose={handleClose}>
          <Alert onClose={handleClose} severity="success">
            Documentation {action + "d"} successfully !
          </Alert>
        </Snackbar>
      ) : isSuccess == "false" ? (
        <Snackbar open={open} autoHideDuration={6000} onClose={handleClose}>
          <Alert onClose={handleClose} severity="error">
            Couldn't {action} documentation !
          </Alert>
        </Snackbar>
      ) : (
        ""
